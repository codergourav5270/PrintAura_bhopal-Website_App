...
export const DEFAULT_SITE_SETTINGS = {
  ...
  admin_settings: {
    enquiriesEmail: 'printaurabhopal@gmail.com',
    helpEmail: 'printaurabhopal@gmail.com',
    directPhones: ['7869014601', '7999830083'],
  },
}

// ✅ NO CACHE — Always fetches fresh data from Supabase
...
// ✅ Always fetches fresh from Supabase — no cache
export async function fetchSiteSettings() {
  const config = await loadAllSiteConfigFromDb()
  return mergeSiteSettingsRow(config?.settingsRow ?? null)
}
...
export async function fetchHeroSettings() {
  ...
  if (config?.heroRow) {
    return { ...EMPTY_HERO, ...config.heroRow }
  }
  return { ...EMPTY_HERO }
}
...
export async function saveAdminSettingsRow(payload) {
  ...
  if (existing?.id) {
    const next = {
      upi_id: payload.upi_id !== undefined ? payload.upi_id : existing.upi_id,
      payment_scanner:
